"""
Audio-only Model Training Script
ViT-Spec (Audio-only, multilingual joint) 훈련 스크립트
"""
...
from models import AudioOnlyModel, compute_metrics, compute_language_specific_metrics
...
def train_epoch(model: nn.Module, 
                ...
                scheduler,
                device: str,
                config: AudioOnlyConfig) -> Dict[str, float]:
    """한 에포크 훈련"""
...
def validate_epoch(model: nn.Module,
                   val_loader,
                   device: str,
                   config: AudioOnlyConfig) -> Tuple[Dict[str, float], Dict[str, Dict[str, float]]]:
    """검증"""
...
def train_model(config: AudioOnlyConfig) -> Tuple[nn.Module, str]:
    ...
    print("🔥 Audio-only (ViT-Spec) 모델 훈련 시작")
    ...
    if config.log_wandb:
        wandb.init(
            ...
            name=f"audio-only-{'-'.join(config.languages)}-{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    ...
    for epoch in range(config.num_epochs):
        ...
        if current_metric > best_metric:
            ...
            if config.save_checkpoints:
                ...
                best_model_path = os.path.join(config.output_dir, 'best_audio_only_model.pth')
                torch.save({
    ...
    print(f"\n✅ Audio-only 모델 훈련 완료! 베스트 {config.best_model_metric}: {best_metric:.4f}")
    return model, best_model_path

def main():
    parser = argparse.ArgumentParser(description="Audio-only Model Training")
    ...
    # 모델 관련
    parser.add_argument('--model_name', type=str, default='google/vit-base-patch16-224', help='ViT 모델명')
