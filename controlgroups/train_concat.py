"""
Concat Model Training Script
Concat (ViT + XLM-R) Late Fusion 훈련 스크립트
"""
...
from config import ConcatConfig
from models import ConcatModel, compute_metrics, compute_language_specific_metrics
...
def train_epoch(model: nn.Module, 
                ...
                scheduler,
                device: str,
                config: ConcatConfig) -> Dict[str, float]:
    """한 에포크 훈련"""
...
def validate_epoch(model: nn.Module,
                   val_loader,
                   device: str,
                   config: ConcatConfig) -> Tuple[Dict[str, float], Dict[str, Dict[str, float]]]:
    """검증"""
...
def train_model(config: ConcatConfig) -> Tuple[nn.Module, str]:
    """모델 훈련"""
    
    print("🔥 Concat (ViT + XLM-R) Late Fusion 모델 훈련 시작")
    print(f"  언어: {config.languages}")
    print(f"  오디오 인코더: {config.audio_encoder}")
    print(f"  텍스트 인코더: {config.text_encoder}")
    print(f"  Fusion 방식: {config.fusion_method}")
    ...
    train_loader, val_loader, test_loader = create_dataloaders(
        config, mode="multimodal", tokenizer=tokenizer, processor=processor
    )
    
    # 모델 생성
    model = ConcatModel(config).to(device)
    ...
    if config.log_wandb:
        wandb.init(
            ...
            name=f"concat-{'-'.join(config.languages)}-{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    ...
    for epoch in range(config.num_epochs):
        ...
        if current_metric > best_metric:
            ...
            if config.save_checkpoints:
                ...
                best_model_path = os.path.join(config.output_dir, 'best_concat_model.pth')
                torch.save({
    ...
    print(f"\n✅ Concat 모델 훈련 완료! 베스트 {config.best_model_metric}: {best_metric:.4f}")
    return model, best_model_path

def main():
    parser = argparse.ArgumentParser(description="Concat Model Training")
    ...
    # 모델 관련
    parser.add_argument('--audio_encoder', type=str, default='google/vit-base-patch16-224', help='오디오 인코더 모델명')
    parser.add_argument('--text_encoder', type=str, default='xlm-roberta-base', help='텍스트 인코더 모델명')
    parser.add_argument('--fusion_method', type=str, default='concat', choices=['concat', 'add'], help='Fusion 방식')
