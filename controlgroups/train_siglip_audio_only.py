...
from config import AudioOnlyConfig
from siglip_control_models import SigLIPAudioOnlyModel, compute_metrics, compute_language_specific_metrics
...
def train_epoch(model: nn.Module, train_loader: DataLoader, optimizer: optim.Optimizer, 
                config: AudioOnlyConfig, scheduler=None) -> Tuple[float, Dict[str, float]]:
    """한 에포크 훈련"""
...
def validate_epoch(model: nn.Module, val_loader: DataLoader, config: AudioOnlyConfig) -> Tuple[float, Dict[str, float], Dict[str, Dict[str, float]]]:
    """한 에포크 검증"""
...
def train_model(config: AudioOnlyConfig) -> Tuple[nn.Module, str]:
    """모델 훈련"""
    
    print("🔥 SigLIP-Audio-Only 모델 훈련 시작")
    ...
    train_loader, val_loader, test_loader = create_dataloaders(
        config, mode="audio_only", processor=processor
    )
    
    # 모델 생성 및 멀티 GPU 설정
    model = SigLIPAudioOnlyModel(config).to(device)
    ...
    if config.log_wandb:
        wandb.init(
            ...
            name=f"siglip-audio-only-{'_'.join(config.languages)}",
            config={
                "model_type": "siglip_audio_only",
    ...
    for epoch in range(config.num_epochs):
        ...
        if current_metric > best_metric:
            ...
            best_model_path = os.path.join(config.output_dir, f"best_siglip_audio_only_{'_'.join(config.languages)}.pt")
    ...
    print(f"\n✅ SigLIP-Audio-Only 모델 훈련 완료! 베스트 {config.best_model_metric}: {best_metric:.4f}")
    return model, best_model_path

def main():
    parser = argparse.ArgumentParser(description="SigLIP-Audio-Only 모델 훈련")
