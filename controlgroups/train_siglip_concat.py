...
from siglip_control_models import SigLIPConcatModel, compute_metrics, compute_language_specific_metrics
...
def train_epoch(model: nn.Module, train_loader: DataLoader, optimizer: optim.Optimizer, 
                config: ConcatConfig, scheduler=None) -> Tuple[float, Dict[str, float]]:
    """한 에포크 훈련"""
    ...
    for batch in tqdm(train_loader, desc="훈련"):
        ...
        # 순전파
        logits = model(pixel_values, input_ids, attention_mask, pixel_attention_mask, spatial_shapes)
...
def validate_epoch(model: nn.Module, val_loader: DataLoader, config: ConcatConfig) -> Tuple[float, Dict[str, float], Dict[str, Dict[str, float]]]:
    """한 에포크 검증"""
...
def train_model(config: ConcatConfig) -> Tuple[nn.Module, str]:
    ...
    # 모델 생성 및 멀티 GPU 설정
    model = SigLIPConcatModel(config).to(device)
    ...
    if config.log_wandb:
        wandb.init(
            ...
            name=f"siglip-concat-{'_'.join(config.languages)}",
            config={
                "model_type": "siglip_concat",
    ...
    for epoch in range(config.num_epochs):
        ...
        if current_metric > best_metric:
            ...
            best_model_path = os.path.join(config.output_dir, f"best_siglip_concat_{'_'.join(config.languages)}.pt")
    ...
    print(f"\n✅ SigLIP-Concat 모델 훈련 완료! 베스트 {config.best_model_metric}: {best_metric:.4f}")
    return model, best_model_path

def main():
    parser = argparse.ArgumentParser(description="SigLIP-Concat 모델 훈련")
