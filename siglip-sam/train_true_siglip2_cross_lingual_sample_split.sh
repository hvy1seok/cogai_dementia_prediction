#!/bin/bash
# 진정한 SigLIP2 - Cross-lingual Zero-shot 훈련 (샘플 단위 분할)
# EMA Teacher-Student + Self-Distillation + Caption Generation
# 환자 단위가 아닌 파일(샘플) 단위로 분할하여 더 많은 학습 데이터 확보

echo "=== 진정한 SigLIP2 - Cross-lingual Zero-shot 훈련 시작 (샘플 단위 분할) ==="
...
case $EXPERIMENT_NUM in
    ...
    4)
        ...
        ;;
    all)
        echo "🔄 모든 Cross-lingual 실험 순차 실행 (샘플 단위 분할)..."
        for i in {1..4}; do
            echo ""
            echo "========================================"
            echo "실험 $i 시작..."
...
OUTPUT_DIR="../modules/outputs/siglip-sam/True_SigLIP2_CrossLingual_Sample_Split_Train_${TRAIN_LANGS_STR}_Test_${TEST_LANGUAGE}"

mkdir -p "$OUTPUT_DIR"
mkdir -p "$OUTPUT_DIR/checkpoints"

echo ""
echo "🔥 진정한 SigLIP2 - Cross-lingual Zero-shot 훈련 설정 (샘플 단위 분할):"
...
echo ""
echo "✨ Cross-lingual + 샘플 단위 분할 핵심 기능:"
...
echo "  📊 기대: 샘플 단위 분할로 더 robust한 language-agnostic representation 학습"
echo ""
...
echo "🚀 진정한 SigLIP2 - Cross-lingual Zero-shot 모델 훈련 시작 (샘플 단위 분할)..."
echo "⚡ EMA Teacher-Student + Multi-Loss + 샘플 단위 분할로 최고 Zero-shot 성능 달성!"
echo "================================"

# 훈련 실행 (Cross-lingual + 샘플 단위 분할 모드)
...
if [ $? -eq 0 ]; then
    echo ""
    echo "✅ 진정한 SigLIP2 - Cross-lingual Zero-shot 모델 훈련 완료! (샘플 단위 분할)"
    ...
    echo ""
    echo "🔥 진정한 SigLIP2 + Cross-lingual + 샘플 단위 분할 핵심 성과:"
    ...
    echo ""
    echo "📊 샘플 단위 분할 Cross-lingual 기대 효과:"
