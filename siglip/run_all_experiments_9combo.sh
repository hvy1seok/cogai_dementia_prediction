...
echo "=== SigLIP2 다국어 모델 9가지 조합 실험 시작 ==="
...
# 실험 카운터
EXPERIMENT_COUNT=0
TOTAL_EXPERIMENTS=$((${#LOSS_TYPES[@]} * ${#OPTIMIZERS[@]}))
...
for loss_type in "${LOSS_TYPES[@]}"; do
    for optimizer in "${OPTIMIZERS[@]}"; do
        ...
        if [ -d "$CHECKPOINT_DIR" ] && [ "$(ls -A $CHECKPOINT_DIR)" ]; then
            BEST_MODEL=$(ls "$CHECKPOINT_DIR"/*best-auc*.ckpt 2>/dev/null | head -1)
...
echo ""
echo "🔬 9가지 조합 실험이 모두 완료되었습니다!"
echo "각 조합의 성능을 비교하여 최적의 설정을 찾아보세요! 🎯"
