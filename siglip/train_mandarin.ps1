# SigLIP2 중국어(만다린) 치매 진단 모델 훈련 (PowerShell)

Write-Host "=== SigLIP2 중국어(만다린) 치매 진단 모델 훈련 시작 ===" -ForegroundColor Green
Write-Host "시작 시간: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"

# 설정
$DATA_DIR = "../training_dset"
$OUTPUT_DIR = "../modules/outputs/siglip/Mandarin"
...
Write-Host "중국어(만다린) 모델 훈련 시작..." -ForegroundColor Green
Write-Host "================================"
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host ""
        Write-Host "✅ 중국어(만다린) 모델 훈련이 성공적으로 완료되었습니다!" -ForegroundColor Green
        Write-Host "완료 시간: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
        Write-Host "모델 저장 위치: $OUTPUT_DIR/checkpoints"
    } else {
        Write-Host ""
        Write-Host "❌ 중국어(만다린) 모델 훈련 중 오류가 발생했습니다." -ForegroundColor Red
        exit 1
    }
} catch {
