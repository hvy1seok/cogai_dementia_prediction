#!/bin/bash
# SigLIP2 중국어(만다린) 치매 진단 모델 훈련

echo "=== SigLIP2 중국어(만다린) 치매 진단 모델 훈련 시작 ==="
...
# 훈련 시작 확인
read -p "중국어(만다린) 모델 훈련을 시작하시겠습니까? (y/N): " confirm
if [[ ! $confirm =~ ^[Yy]$ ]]; then
    echo "훈련이 취소되었습니다."
    exit 0
fi

echo "중국어(만다린) 모델 훈련 시작..."
echo "================================"
...
if [ $? -eq 0 ]; then
    echo ""
    echo "✅ 중국어(만다린) 모델 훈련이 성공적으로 완료되었습니다!"
    echo "완료 시간: $(date '+%Y-%m-%d %H:%M:%S')"
    echo "모델 저장 위치: $OUTPUT_DIR/checkpoints"
else
    echo ""
    echo "❌ 중국어(만다린) 모델 훈련 중 오류가 발생했습니다."
    exit 1
fi
