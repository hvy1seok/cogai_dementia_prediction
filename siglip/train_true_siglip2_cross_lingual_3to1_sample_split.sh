#!/bin/bash
# 진정한 SigLIP2 - 3개 언어 Cross-lingual Zero-shot 훈련 (샘플 단위 분할, PyTorch Lightning)
# 영어, 만다린, 스페인어 중 2개로 훈련하고 1개로 테스트 (그리스어 제외)
# EMA Teacher-Student + Self-Distillation + Caption Generation

echo "=== 진정한 SigLIP2 - 3개 언어 Cross-lingual Zero-shot 훈련 시작 (샘플 단위 분할, PyTorch Lightning) ==="
...
case $EXPERIMENT_NUM in
    ...
    3)
        ...
        ;;
    all)
        echo "🔄 모든 3개 언어 Cross-lingual 실험 순차 실행 (샘플 단위 분할, PyTorch Lightning)..."
        for i in {1..3}; do
            echo ""
            echo "========================================"
            echo "실험 $i 시작..."
...
OUTPUT_DIR="../modules/outputs/siglip/True_SigLIP2_3Lang_CrossLingual_Sample_Split_Lightning_Train_${TRAIN_LANGS_STR}_Test_${TEST_LANGUAGE}"

mkdir -p "$OUTPUT_DIR"
mkdir -p "$OUTPUT_DIR/checkpoints"

echo ""
echo "🔥 진정한 SigLIP2 - 3개 언어 Cross-lingual Zero-shot 훈련 설정 (샘플 단위 분할, PyTorch Lightning):"
...
echo ""
echo "✨ 3개 언어 Cross-lingual + 샘플 단위 분할 + PyTorch Lightning 핵심 기능:"
...
echo "  📊 기대: 샘플 단위 분할 + Lightning + 핵심 언어로 최고 Zero-shot 성능"
...
echo "🚀 진정한 SigLIP2 - 3개 언어 Cross-lingual Zero-shot 모델 훈련 시작 (샘플 단위 분할, PyTorch Lightning)..."
echo "⚡ EMA Teacher-Student + Multi-Loss + Lightning + 샘플 단위 분할로 최고 Zero-shot 성능 달성!"
echo "================================"

# 훈련 실행 (3개 언어 Cross-lingual + 샘플 단위 분할 + PyTorch Lightning 모드)
...
if [ $? -eq 0 ]; then
    echo ""
    echo "✅ 진정한 SigLIP2 - 3개 언어 Cross-lingual Zero-shot 모델 훈련 완료! (샘플 단위 분할, PyTorch Lightning)"
    ...
    echo "  제외 언어: Greek"
    echo ""
    echo "🔥 진정한 SigLIP2 + 3개 언어 Cross-lingual + 샘플 단위 분할 + PyTorch Lightning 핵심 성과:"
    ...
    echo ""
    echo "📊 3개 언어 + 샘플 단위 분할 + Lightning Cross-lingual 기대 효과:"
    echo "   🎯 그리스어 제외로 데이터 품질 집중"
