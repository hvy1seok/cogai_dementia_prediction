#!/bin/bash
# 진정한 SigLIP2 - 영어/중국어 Cross-lingual Zero-shot 훈련 (PyTorch Lightning)
# 스페인어, 그리스어 제외 - 학습이 잘 되는 언어만 사용
# EMA Teacher-Student + Self-Distillation + Caption Generation

echo "=== 진정한 SigLIP2 - 영어/중국어 Cross-lingual Zero-shot 훈련 시작 (PyTorch Lightning) ==="
...
case $EXPERIMENT_NUM in
    ...
    2)
        ...
        ;;
    all)
        echo "🔄 모든 영어/중국어 Cross-lingual 실험 순차 실행 (PyTorch Lightning)..."
        for i in {1..2}; do
            echo ""
            echo "========================================"
            echo "실험 $i 시작..."
...
OUTPUT_DIR="../modules/outputs/siglip/True_SigLIP2_CrossLingual_Lightning_EN_CN_Train_${TRAIN_LANGS_STR}_Test_${TEST_LANGUAGE}"

mkdir -p "$OUTPUT_DIR"
mkdir -p "$OUTPUT_DIR/checkpoints"

echo ""
echo "🔥 진정한 SigLIP2 - 영어/중국어 Cross-lingual Zero-shot 훈련 설정 (PyTorch Lightning):"
...
echo ""
echo "✨ 영어/중국어 Cross-lingual + PyTorch Lightning 핵심 기능:"
...
echo "🚀 진정한 SigLIP2 - 영어/중국어 Cross-lingual Zero-shot 모델 훈련 시작 (PyTorch Lightning)..."
echo "⚡ EMA Teacher-Student + Multi-Loss + Lightning + 핵심 언어로 최고 Zero-shot 성능 달성!"
echo "================================"

# 훈련 실행 (Cross-lingual + 환자 단위 분할 + PyTorch Lightning 모드)
...
if [ $? -eq 0 ]; then
    echo ""
    echo "✅ 진정한 SigLIP2 - 영어/중국어 Cross-lingual Zero-shot 모델 훈련 완료! (PyTorch Lightning)"
    ...
    echo ""
    echo "🔥 진정한 SigLIP2 + 영어/중국어 Cross-lingual + PyTorch Lightning 핵심 성과:"
    ...
    echo ""
    echo "📊 영어/중국어 Cross-lingual + Lightning 기대 효과:"
